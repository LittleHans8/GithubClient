...
import littlehans.cn.githubclient.network.Pagination.PageEmitter;
import littlehans.cn.githubclient.network.Pagination.PagePaginatorContract;
import littlehans.cn.githubclient.network.callback.PaginatorCallback;
import okhttp3.Headers;
import retrofit2.Call;

public class PagePaginator<T> extends Paginator<T>
  ...
  @Override protected void processPage(T dataArray) {
    //mHasMore = dataArray.size() >= mPerPage; // 判断是否还有更多的数据 数据的数量是否大于规定每页的数据量。
  ...
  @Override public void respondHeader(Headers headers) {
    String link = headers.values("Link").get(0);

    mLastPage = new PageLink(link).getLastPage();
    if (mLastPage == 0) {
